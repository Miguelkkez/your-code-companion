...
export default function OrderCard({ order, onUpdate }: OrderCardProps) {
  ...
  return (
    <>
      <div className={`bg-card rounded-2xl border border-border p-5 shadow-sm hover:shadow-md transition-all duration-300 ${isCancelled ? "opacity-50" : ""}`}>
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            {!isCancelled && (
              <button onClick={() => setEditOpen(true)} className="p-1.5 rounded-lg hover:bg-muted transition-colors" title="Editar pedido">
      ...
      <EditOrderDialog order={order} open={editOpen} onOpenChange={setEditOpen} onUpdate={() => onUpdate?.()} />
    </>
  );
}
